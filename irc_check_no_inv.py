...
def get_ldap_configuration(**output_dict):
    ...
    if 'result' not in response_json.keys() and denied_error in response_json:
        ...
        group_search_base_dn = ldap_result['groupSearchBaseDN']
        group_search_filter = ldap_result['groupSearchCustomFilter']
        group_search_type = ldap_result['groupSearchType']
...
def get_filename(mvip):
    ...
    out_path = "./output_files/" + filename
    if os.path.exists(out_path):
        os.remove(out_path)
    print('Output file name is: {}'.format(out_path))
    return out_path
